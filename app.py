...
if ref_f1 and svr_f1:
    ...
    with c1: ref_code_col = st.selectbox("Kendi Listeniz: Malzeme Kodu", df_ref_f1.columns, key="f1_c1")
    with c2: svr_code_col = st.selectbox("SVR: Malzeme Kodu", df_svr_f1.columns, key="f1_c2")
    with c3: svr_name_col = st.selectbox("SVR: Malzeme Adı", df_svr_f1.columns, key="f1_c3")
    with c4: svr_price_col = st.selectbox("SVR: Birim Fiyatı (Tanımlı)", df_svr_f1.columns, key="f1_c4")
...
# ==============================================================================
# 2. BÖLÜM: STOK KARŞILAŞTIRMA VE MANUEL ASİSTAN
# ==============================================================================
...
if file_s1 and file_s2:
    ...
    with sc1: r_c = st.selectbox("Stok: Kod", df_s1.columns, key="s2_c1")
    with sc2: r_n = st.selectbox("Stok: İsim", df_s1.columns, key="s2_c2")
    with sc3: c_c = st.selectbox("Karşı: Kod", df_s2.columns, key="s2_c3")
    with sc4: c_n = st.selectbox("Karşı: İsim", df_s2.columns, key="s2_c4")
    if st.button("🚀 Otomatik Eşleştir", key="s2_btn"):
    ...
    if 'not_found' in st.session_state:
        ...
        with t1:
            ...
            if all_m: st.dataframe(pd.DataFrame(all_m), use_container_width=True)
        with t2:
...
# ==============================================================================
# 3. BÖLÜM: KARLILIK ANALİZİ VE YENİDEN FİYATLANDIRMA
# ==============================================================================
...
with col_k1: file_k1 = st.file_uploader("1. Excel (Tanımlı Fiyat Olan)", type="xlsx", key="k_ref")
with col_k2: file_k2 = st.file_uploader("2. Excel (Karşı Net Fiyat Olan)", type="xlsx", key="k_comp")

if file_k1 and file_k2:
    df_k1, df_k2 = pd.read_excel(file_k1), pd.read_excel(file_k2)
    sk1, sk1_name, sk2, sk3, sk4 = st.columns(5)
    with sk1: k1_code = st.selectbox("Excel 1: Malzeme Kodu", df_k1.columns, key="k3_c1")
    with sk1_name: k1_name_col = st.selectbox("Excel 1: Ürün Adı", df_k1.columns, key="k3_name")
    with sk2: k1_price = st.selectbox("Excel 1: Tanımlı Fiyat", df_k1.columns, key="k3_c2")
    with sk3: k2_code = st.selectbox("Excel 2: Malzeme Kodu", df_k2.columns, key="k3_c3")
    with sk4: k2_price = st.selectbox("Excel 2: Karşı_Net Fiyat", df_k2.columns, key="k3_c4")
        ...
        for _, row in df_k1.iterrows():
            ...
            if c in cost_map:
                ...
                k_results.append({
                    ...
                    "Ürün Adı": row[k1_name_col], # Yeni eklenen sütun
